...
SELECTED_PROJECTS=$(tmuxinator list -n |
    tail -n +2 |
    fzf --prompt="Project: " -m -1 -q "$1" --layout=reverse --border --info=inline --margin=8,20 --padding=1)
...
if [ -n "$SELECTED_PROJECTS" ]; then
    ...
    if [ -n "$TMUX" ]; then
        SESSION=$(tmux list-sessions -F "#S" | fzf --prompt="Session: " --layout=reverse --border --info=inline --margin=8,20 --padding=1)
        if [ -n "$SESSION" ]; then
